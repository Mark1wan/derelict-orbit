...
func _init() -> void:
	...
	var wet := CaveTex.limestone(256, Color(0.40, 0.40, 0.39))
	var dry := CaveTex.limestone()

	# The wall of the whole cave. Darker than instinct says it should be: a beam on rock at two
	# metres is already bright, and anything lighter blows out. Low roughness, so the beam
	# catches highlights on it the way it does on rock with a film of water.
	mats["rock_wet"] = textured(wet, Color(0.44, 0.44, 0.45), SCALE["rock"], 0.46, 0.0, 1.6)

	# Dry bedded limestone. Nothing in Sowbelly asks for it; it is the fallback `get_mat`
	# returns for an unknown key, and the next cave may want somewhere that is not wet.
	mats["rock"] = textured(dry, Color(0.58, 0.56, 0.52), SCALE["rock"], 0.92, 0.0, 1.5)
	...
	mats["flow"] = textured(wet, Color(0.46, 0.46, 0.46), SCALE["rock"], 0.42, 0.0, 1.6)
	...
	var seam := textured(wet, Color(0.42, 0.42, 0.43), SCALE["rock"], 0.48, 0.0, 1.6)
